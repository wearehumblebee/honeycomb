...
const createDotenvFile = ({ directory, exampleFilePath, targetFilePath }: CreateDotenvArguments): void => {
  ...
  try {
    ...
    console.info(`🙌 A local "${targetFilePath}" file has been created from the provided example "${exampleFilePath}"`);
  } catch (error) {
    ...
    if (error.code !== 'EEXIST') {
      console.warn(`⚠️ [WARN] Unable to create local file "${targetFilePath}" based on example "${exampleFilePath}".`);
      console.error(error);
    }
  }
};
