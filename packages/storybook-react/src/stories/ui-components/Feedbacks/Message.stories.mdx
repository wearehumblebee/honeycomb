...
<Meta title="UI Components / Feedbacks / Message" component={Message} parameters={{ layout: 'centered' }} />
...
<Preview>
  <Story name="Default">
    {() => {
      ...
      return (
        <Message variant={select('Variant', theme ? Object.keys(theme.messages || {}) : [], 'primary')}>
          Everyone&apos;s a genius. But if you judge a fish by its ability to climb a tree, it will live its whole life
          believing that it is stupid.
          <br />
          <a href="https://quoteinvestigator.com/2013/04/06/fish-climb/" target="_blank" rel="noopener noreferrer">
            Albert Einstein(?)
          </a>
        </Message>
      );
