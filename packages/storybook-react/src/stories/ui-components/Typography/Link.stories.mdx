...
<Meta title="UI Components / Typography / Link" component={Link} parameters={{ layout: 'centered' }} />
    ...
    {() => {
      ...
      return (
        <Link
          ...
          variant={select('Variant', theme && theme.links ? Object.keys(theme.links || {}) : [], 'styles.a')}
          onClick={onClick}
        >
