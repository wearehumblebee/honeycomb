...
<Meta title="UI Components / Forms / Slider" component={Slider} parameters={{ layout: 'centered' }} />
...
<Preview>
  <Story name="State">
    {() => {
      ...
      return (
        <React.Fragment>
          <Label htmlFor="sliders.state">On a scale of 1 to 10, how stressed are you feeling right now?</Label>
          <Slider
            id="sliders.state"
            min={0}
            max={10}
